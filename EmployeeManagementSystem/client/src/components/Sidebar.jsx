...
import { MenuIcon, UserIcon, XIcon, LayoutGridIcon, CalendarIcon, 
  FileTextIcon, DollarSignIcon, SettingsIcon, ChevronRightIcon, 
...
//Close mobile sidebar when route changes
              ...
              <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                {navItems.map((item) => {
                  ...
                  return (
                    ...
                    gap-3 px-3 py-2.5 text-[13px] font-medium transition-all duration-150 relative
  ...
  return (
    <>
       ...
       <button onClick={() => setMobileMenuOpen(true)} className='lg:hidden fixed top-4 left-4 z-50 p-2 bg-slate-900  
       text-white rounded-lg shadow-lg border border-white/10'>
        <MenuIcon size={20}/>
       </button>
       {/*Mobile overlay*/}
       {mobileMenuOpen && <div className='lg:hidden fixed inset-0 z-40 bg-black/50' 
       onClick={() => setMobileMenuOpen(false)}/>}
       {/*Sidebar - Desktop */}
       ...
       via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4'>
         ...
         duration-300 ${mobileMenuOpen ? ' translate-x-0' : ' -translate-x-full'}`}>
          {sidebarContent}
         </aside>
